...
def max_decode_v2(Y_pred: torch.Tensor) -> torch.Tensor:
    """Decode the predicted alignment matrix by taking the maximum value for each audio frame.
    ...
    Y_pred_binary[torch.arange(N).unsqueeze(-1), torch.arange(E), max_idxs] = 1
...
def DTW_v2(Y_pred: torch.Tensor) -> torch.Tensor:
    """Decode the predicted alignment matrix using a modified version of dynamic time warping.
    These modifications include:
    - Using a negative cost for alignment probabilities
    - Allowing downward motion in the warping path, such that each midi score onset can be aligned to multiple audio frames
    - Allowing the warping path to start and end anywhere in the score, since the audio clip only contains a segment of the score
    
    Args:
        Y_pred (torch.Tensor): Predicted probability alignment matrix of shape (N, E, X), where N is the batch size, E is the number of MIDI events in the score and X is the number of audio frames.
    ...
    for n in range(N):
        ...
        while i > 0 and j > 0:
            dtw_path[n, i - 1, j - 1] = 1
            if i > 1 and D[i - 1, j - 1] <= D[i, j - 1] and D[i - 1, j - 1] <= D[i - 1, j]:
                i -= 1
                j -= 1
            elif i > 1 and D[i - 1, j] <= D[i, j - 1] and D[i - 1, j] <= D[i - 1, j - 1]:
                i -= 1
            else:
                j -= 1

...
def temporal_distance(Y_pred: torch.Tensor, Y: torch.Tensor, midi_event_timestamps, tau: float = 0.) -> torch.Tensor:
    ...
    Args:
        ...
        tau (float): Threshold for alignment distance.
    ...
    L1_distances = torch.abs(pred_timestamps - true_timestamps) - tau
    threshold_distances = L1_distances * (L1_distances > 0).float()

    return torch.mean(threshold_distances)


def temporal_distance_vec(Y_pred: torch.Tensor, Y: torch.Tensor, midi_event_timestamps, tau: float) -> torch.Tensor:
    """Compute the audio-frame-wise temporal alignment distance between the predicted and ground-truth binary alignment matrices.
...
def binary_accuracy(Y_pred: torch.Tensor, Y: torch.Tensor, midi_event_timestamps: torch.Tensor, tau: float) -> torch.Tensor:
    ...
    binary_accuracies = (torch.abs(pred_timestamps - true_timestamps) <= tau).float()
    return torch.mean(binary_accuracies)


def binary_accuracy_vec(Y_pred: torch.Tensor, Y: torch.Tensor, midi_event_timestamps: torch.Tensor, tau: float) -> torch.Tensor:
    """Compute the audio-frame-wise binary alignment accuracy between the predicted and ground-truth binary alignment matrices.
...
def monotonicity(Y_pred: torch.Tensor, midi_event_timestamps: torch.Tensor) -> bool:
...
def monotonicity_vec(Y_pred: torch.Tensor, midi_event_timestamps: torch.Tensor) -> torch.Tensor:
